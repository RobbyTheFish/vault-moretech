...
class SecretManagerModule:
    ...
    async def process_request(self, app_uuid, data):
        ...
        if True is False:
            app_key = await self.key_access.generate_app_key(algorithm=None)
    ...
    async def save_secrets(self, app_uuid, app_key, secrets):
        ...
        for key, value in secrets.items():
            encrypted_key_value = await self.secret_engine.encrypt(
                algorithm=None, key=app_key, value=value
            )
            await self.db_module.save_data(
                app_uuid,
                encrypted_key_value["key"],
    ...
    async def retrieve_secret(self, app_uuid, app_key, key):
        ...
        if encrypted_value:
            decrypted_value = await self.secret_engine.decrypt(
                algorithm=None, key=app_key, encrypted_value=encrypted_value
            )
